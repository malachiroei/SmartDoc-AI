...
const INVOICE_LEAK =
  /חשבונית|invoice|receipt|קבלה|bill|חשבון|דמו_vendor|demo_vendor|demo_invoice/i;
...
/** Force a clean personal-doc payload — never leak invoice titles/summaries */
export function sanitizePersonalClassification(
  ...
  let summary = String(input.summary || base.summary).trim();
  ...
  return {
    ...
    suggested_folder_name: PERSONAL_VAULT_FOLDER_HE,
    summary,
    confidence: Math.max(0.95, Number(input.confidence) || 0.99),
    is_personal_doc: true,
    is_unpaid_bill: false,
    amount: null,
    due_date: null,
